	...
	<div class="mx-auto max-w-md px-5 py-16 text-center">
		...
		<template v-else-if="isConfirmed">
			...
			<p class="mt-2 text-p-sm text-ink-gray-6">
				{{ __("You're all set. We'll email you the details.") }}
			</p>
...
const breadcrumbs = [
	{ label: __('1:1 Sessions'), route: { name: 'SlotPicker' } },
	{
		...
		route: { name: 'BookingPaymentResult', params: { name: props.name } },
	},
]
...
function retry() {
	router.push({ name: 'BookingCheckout', params: { name: props.name } })
}
</script>
